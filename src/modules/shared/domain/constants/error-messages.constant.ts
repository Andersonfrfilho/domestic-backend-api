export const ErrorMessages = {
  ...
  'decimal.min': (
    ...
    decimal_digits_limit: number | null = null,
  ) =>
    `O campo ${param} deve ser maior ou igual a ${limit?.toFixed(decimal_digits_limit || 0) ?? '-'}.`,
  'decimal.max': (
    param: string,
    limit: number | null = null,
    decimal_digits_limit: number | null = null,
  ) =>
    `O campo ${param} deve ser menor ou igual a ${limit?.toFixed(decimal_digits_limit || 0) ?? '-'}.`,
  'boolean.base': (param: string) => `O campo ${param} deve ser do tipo booleano.`,
  'date.base': (param: string) => `O campo ${param} deve ser uma data válida.`,
