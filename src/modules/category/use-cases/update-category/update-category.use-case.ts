...
import { UPDATE_CATEGORY_LOG_MESSAGES } from './update-category.constants';
import {
...
export class UpdateCategoryUseCase implements UpdateCategoryUseCaseInterface {
  ...
  async execute(params: UpdateCategoryUseCaseParams): Promise<UpdateCategoryUseCaseResponse> {
    ...
    this.logProvider.info({
      message: UPDATE_CATEGORY_LOG_MESSAGES.START_FLOW,
      context: this.logContext,
      meta: { categoryId: id, fields: Object.keys(updateData) },
    });

    const existing = await this.categoryRepository.findById(id);
    if (!existing) {
      this.logProvider.warn({
        message: UPDATE_CATEGORY_LOG_MESSAGES.CATEGORY_NOT_FOUND,
        context: this.logContext,
        meta: { categoryId: id },
      });
    ...
    if (updateData.slug && updateData.slug !== existing.slug) {
      ...
      if (slugExists) {
        this.logProvider.warn({
          message: UPDATE_CATEGORY_LOG_MESSAGES.DUPLICATE_SLUG,
          context: this.logContext,
          meta: { categoryId: id, slug: updateData.slug },
        });
    ...
    this.logProvider.info({
      ...
      context: this.logContext,
      meta: { categoryId: updated.id, slug: updated.slug },
    });
